...
const getUserByFeed = `-- name: GetUserByFeed :one

SELECT users.id, users.created_at, users.updated_at, users.name from feeds
join users on feeds.user_id =users.id
WHERE feeds.id = $1 LIMIT 1
`

func (q *Queries) GetUserByFeed(ctx context.Context, id uuid.UUID) (User, error) {
	row := q.db.QueryRowContext(ctx, getUserByFeed, id)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
